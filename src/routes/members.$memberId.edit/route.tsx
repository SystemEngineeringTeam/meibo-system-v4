...
// 仮データ: メンバーIDに基づく初期値
function getDefaultValues(_memberId: string | undefined): Partial<RegistrationFormData> {
  // 仮データとして、memberIdに基づいて異なるデータを返す
  const defaultData: Partial<RegistrationFormData> = {
    name: "石丸凜弥",
    furigana: "インマルリンヤ",
    graduationYear: "28卒",
    grade: "B3",
...
export default function Registration(): JSX.Element {
  ...
  const defaultValues = useMemo(() => getDefaultValues(memberId), [memberId]);
  const {
    register,
    handleSubmit,
    watch,
  ...
  const renderLivingWithFamilyField = (label: string): JSX.Element => (
    <div style={{ padding: "20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      <div>
        <p style={{ marginBottom: "8px" }}>{label}</p>
        {errors.isLivingWithFamily != null && (
    ...
    >
      ...
      <div style={{ marginBlock: "40px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <IconButton
          ...
          onClick={() => void navigate(`/members/${memberId}`)}
          type="button"
          variant="outlined"
        >
          <p>戻る</p>
